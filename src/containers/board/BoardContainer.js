...
import { getBoard, changeLists, updateListSeq, updateCardSeq, changeModalVisible } from '../../modules/board';
...
const BoardContainer = ({ match: {params}}) => {
    ...
    const onDragEnd = async result => {
        ...
        if (sourceList.list_id === destList.list_id) {
            ...
            onUpdateCardSeq(sourceListId, destListId, source.index, destination.index, 
                            deletedCard[0].card_id, newLists, prevLists);
        } else {
    ...
    const onUpdateListSeq = (listId, sourceListSeq, destListSeq, newLists, prevLists) => {
        ...
        dispatch(updateListSeq({boardId, data}))
        .then((result) => {
    ...
    const onUpdateCardSeq = (sourceListId, destListId, sourceCardSeq, destCardSeq, cardId, newLists, prevLists) => {
        ...
        dispatch(updateCardSeq({boardId, list_id: sourceListId, data}))
        .then((result) => {
            if (result.success === false) {
                dispatch(changeLists(prevLists))
            }
